...
def updateDocente():
    ...
    with psycopg.connect(keys.link_conection_database()) as conn:

        # for archive in archivesCSV:
            df = pd.read_csv(f"{folder}{archivesCSV[1]}")
...
def refreshLattesByCpf():
    with psycopg.connect(keys.link_conection_database()) as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT cpf FROM public.discente")
            coluna_como_lista = [row[0] for row in cur.fetchall()]
            data_atualizacao = datetime.date.today().strftime('%d%m%Y')
            for cpf in coluna_como_lista:
                 
                 #CODIGO DA EXTRAÇÃO DE LATTES PELO CPF
                 #
                 #
                 #
                
                #CODIGO DE ATUALIZAÇÃO DO BANCO DE DADOS
                 cur.execute("UPDATE public.discente \
                                    SET id_lattes = %s, \
                                    data_atualizacao = %s,\
                                    WHERE cpf = %s\
